...
Received: "http://localhost:8080/auth"

Call log:
  - Expect "toHaveURL" with timeout 5000ms
    3 × unexpected value "http://localhost:8080/auth"
    ...
    - generic [ref=e13]:
      ...
      - generic [ref=e17]:
        - generic [ref=e18]:
          ...
          - 'textbox "VD: saber123" [ref=e20]': testuser_7808
        - generic [ref=e21]:
          - generic [ref=e22]: Mật Khẩu
          - textbox "••••••••" [ref=e23]: password123
        - button "Đang Xử Lý..." [disabled] [ref=e24]
      - paragraph [ref=e25]: Tài khoản mới sẽ tự động mở khoá nhân vật Saber và nhận 999,999,999 KC để trải nghiệm.
```
...
> 19 |     await expect(page).toHaveURL(`${BASE_URL}/`);
  ...
  24 |     await page.click('text=Triệu Hồi');
  ...
  36 |     await page.click('text=Túi đồ');
  ...
  46 |     await page.click('text=Nhân vật');
  ...
  55 |     await page.click('text=Trống'); // Assumes empty slot says "Trống" in the UI
  56 |     
  57 |     // Select the first available item in the modal
  58 |     await page.waitForSelector('.fixed.inset-0.bg-black\\/90'); // Modal open
  59 |     const availableItems = page.locator('button:has-text("Gắn")');
  60 |     if (await availableItems.count() > 0) {
  61 |       await availableItems.first().click();
  62 |       
  63 |       // Verify CP increased
  64 |       await page.waitForTimeout(1000); // Wait for hydration/sync
  65 |       const newCpText = await page.locator('text=Lực chiến:').textContent();
  66 |       const newCP = parseInt(newCpText?.replace(/[^0-9]/g, '') || '0');
  67 |       console.log('New CP:', newCP);
  68 |       expect(newCP).toBeGreaterThan(initialCP);
  69 |     } else {
  70 |       console.log('No compatible items found to equip during test.');
  71 |     }
  72 |   });
  73 | 
  74 |   test('should load battle and show combatants', async ({ page }) => {
  75 |     // Reuse the previous state if possible, but Playwright tests are isolated.
  76 |     // For simplicity, we just check if the Battle screen loads.
  ...
  82 | 
  83 |     await page.click('text=Chiến đấu');
  84 |     await expect(page).toHaveURL(`${BASE_URL}/pve`); // Assuming /pve is the battle screen route
  85 |     
  86 |     // Select a level
